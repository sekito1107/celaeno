...
export default class extends Controller {
  ...
  showDetail(event) {
    ...
    if (previewContainer && html) {
      ...
      if (
        previewContainer.dataset.pinnedBy &&
  ...
  selectCard(event) {
    ...
    if (this.selectedCardId === cardId) {
      ...
      const targetTypeHint = element.getAttribute(
        'data-game--card-target-type-hint-value'
      );
  ...
  deselectAll() {
    this.element
      .querySelectorAll('[data-controller~="game--card"]')
      .forEach((el) => {
  ...
  async drop(event) {
    ...
    if (cardType === 'cancel') {
      ...
      if (
        event.currentTarget.closest('.hand-container') ||
        event.currentTarget.classList.contains('hand-container')
      ) {
  ...
  async performCardPlay(cardId, position, targetId = null) {
    try {
      ...
      const response = await api.post(
        `/games/${this.gameIdValue}/card_plays`,
        body
      );
  ...
  async ready(event) {
    ...
    const request = new FetchRequest(
      'post',
      `/games/${this.gameIdValue}/ready_states`,
      {
  ...
  async performCardCancel(cardId) {
    try {
      const response = await api.delete(
        `/games/${this.gameIdValue}/card_plays/${cardId}`
      );
  ...
  _highlightValidTargets(hint) {
    ...
    switch (hint) {
      case 'slot':
        ...
        // または子要素がないスロット
        selector =
          '.play-mat:not(.play-mat-opponent) .field-slot:has(.empty-slot), .play-mat:not(.play-mat-opponent) .field-slot:empty';
        break;
      case 'enemy_unit':
        // 相手のユニット (カードがあるスロット)
      ...
      case 'ally_unit':
        // 自分のユニット
        selector =
          '.play-mat:not(.play-mat-opponent) .field-slot .card-wrapper';
        break;
      case 'any_unit':
        // 敵味方問わず全てのユニット
        selector =
          '.play-mat:not(.play-mat-opponent) .field-slot .card-wrapper, .play-mat-opponent .field-slot .card-wrapper';
        break;
      case 'enemy_board':
        // 相手フィールド全体 (ユニットゾーンのみ)
